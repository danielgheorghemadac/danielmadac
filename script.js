(function () {
    ...
    // ============ FLYING ROCKETS WITH COLLISION FUN FACTS ============
    (function initRockets() {
        var rockets = [];
        var MAX_ROCKETS = 10;
        var spawnIntervals = [4000, 10000, 25000, 50000];
        var spawnIndex = 0;
        var lastFactTime = 0;
        var FACT_COOLDOWN = 35000;
        ...
        function createRocket() {
            ...
            rocket.innerHTML = '<img src="icons/rocket.png" alt="" class="rocket-img">';
            ...
            if (edge === 0) { x = -40; y = Math.random() * vh; vx = speed; vy = (Math.random() - 0.5) * speed; }
            else if (edge === 1) { x = vw + 40; y = Math.random() * vh; vx = -speed; vy = (Math.random() - 0.5) * speed; }
            else if (edge === 2) { x = Math.random() * vw; y = -40; vx = (Math.random() - 0.5) * speed; vy = speed; }
            else { x = Math.random() * vw; y = vh + 40; vx = (Math.random() - 0.5) * speed; vy = -speed; }

            var angle = Math.atan2(vy, vx) * (180 / Math.PI) + 90;
            rocket.style.transform = 'translate(' + x + 'px,' + y + 'px) rotate(' + angle + 'deg)';

            rocketContainer.appendChild(rocket);

            var rObj = { el: rocket, x: x, y: y, vx: vx, vy: vy, angle: angle, alive: true };
            rockets.push(rObj);

            setTimeout(function () { rocket.classList.add('visible'); }, 50);
        }
        ...
        function showFact(x, y) {
            ...
            popup.className = 'rocket-fact';
            popup.innerHTML = '<div class="rocket-fact-q">' + fact.q + '</div><div class="rocket-fact-a">' + fact.a + '</div>';

            popup.style.left = Math.min(Math.max(x - 120, 20), window.innerWidth - 280) + 'px';
            popup.style.top = Math.min(Math.max(y - 60, 20), window.innerHeight - 120) + 'px';
        ...
        function update() {
            ...
            for (var i = rockets.length - 1; i >= 0; i--) {
                ...
                r.x += r.vx;
                r.y += r.vy;
                r.el.style.transform = 'translate(' + r.x.toFixed(1) + 'px,' + r.y.toFixed(1) + 'px) rotate(' + r.angle + 'deg)';

                if (r.x < -80 || r.x > vw + 80 || r.y < -80 || r.y > vh + 80) {
                    r.alive = false;
                    r.el.remove();
                    rockets.splice(i, 1);
                }
        ...
        function spawnLoop() {
            createRocket();
            var delay = spawnIntervals[spawnIndex % spawnIntervals.length];
            spawnIndex++;
